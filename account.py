...
class Account:
    ...
    def get_non_active_friends(self):
        ...
        half_year = 365/2
        friends_list = self.get_public_friends()
        friends_list = filter(lambda friend: (not friend.is_online()) and (
        ...
        non_active_accounts = list(
            filter(lambda user: user.get_days_offline() > half_year, non_active_accounts))
